...
# Launches the Balsam Supervisor, which manages the API lifecycle.
# The Supervisor runs on port 5001 and the API on port 5000.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUPERVISOR="$SCRIPT_DIR/supervisor/Balsam.Supervisor"
...
chmod +x "$SCRIPT_DIR/api/Balsam.API" 2>/dev/null || true
