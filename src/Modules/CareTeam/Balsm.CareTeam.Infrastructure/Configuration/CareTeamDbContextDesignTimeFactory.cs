...
internal sealed class CareTeamDbContextDesignTimeFactory : IDesignTimeDbContextFactory<CareTeamDbContext>
{
    public CareTeamDbContext CreateDbContext(string[] args)
    {
        ...
        var builder = new DbContextOptionsBuilder<CareTeamDbContext>();
        if (provider.Equals("sqlite", StringComparison.OrdinalIgnoreCase))
        {
            builder.UseSqlite(
                "Data Source=design-time.db",
                o => o.MigrationsAssembly("Balsm.CareTeam.Infrastructure.Migrations.Sqlite"));
        }
        else
        {
        ...
        return new CareTeamDbContext(builder.Options, new NullDomainEventDispatcher());
    }
